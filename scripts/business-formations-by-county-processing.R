...
##################################################################
#
# Processing Script for Business Formations by County
...
formations_df_calc <- unique(formations_df %>% 
  ...
  select(County, `County FIPS`, Year, `Measure Type`, Variable, Value) %>% 
  arrange(County) %>% 
...
write.table(
  formations_df_calc,
  file.path(getwd(), "data", "business_formations_county.csv"),
  sep = ",",
  row.names = F,
  na = "-9999"
)
